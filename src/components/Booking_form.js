...
console.log("finalval",finalval);
 ...
 const rent = defcars.find(defcars => defcars.id == finalval);
 const rentprkm = rent.Rent_per_km;
 let cormod = rent.car_model;
 console.log("Rent is",rent);
let amtpaid = rentprkm * (100 * days_diff);
this.setState({amt:amtpaid, carmodel:cormod, days:days_diff});
...
this.setState({dep_amt:dipo_amt});
  ...
  handleSubmit(event) {
    ...
    var data = [
     ...
     console.log("data u",JSON.stringify(data));
